...
class Diklat_m extends MY_Model
{
	...
	{
        $query = $this->db->where('deleted_at',NULL)->order_by('kode', 'ASC')->get('ref_diklat');
        if($query->num_rows() > 0){
        $dropdown[''] = 'Pilih Jenis Diklat Struktural';
		foreach ($query->result() as $row)
		{
			$dropdown[$row->kode] = $row->diklat;
		}
        }else{
            $dropdown[] = 'Belum Ada Diklat Struktural Tersedia'; 
        }
		return $dropdown;
	}
